...
export async function OpenApi() {
  ...
  return (
    <section className="relative overflow-hidden bg-linear-to-b from-ink-void via-[#0c2a6b] to-ink-void py-14 text-white md:py-28 lg:py-32">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <span className="animate-orb-drift absolute top-10 right-[8%] h-64 w-64 rounded-full bg-sky-500/25 blur-[100px]" />
        <span className="animate-orb-drift absolute bottom-16 left-[4%] h-56 w-56 rounded-full bg-primary/35 blur-[90px]" />
      </div>
      ...
      <div className="relative mx-auto grid w-full max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-12 lg:gap-16 lg:px-8">
        <Reveal className="lg:col-span-5">
          ...
          <p className="mt-5 max-w-lg text-base leading-relaxed text-sky-50/85 md:text-lg">
            {t("body")}
          </p>
          ...
          <ul className="mt-8 grid gap-3 sm:grid-cols-2">
            {CAPABILITIES.map((cap, index) => (
              <li
                key={cap}
                className="group flex items-start gap-3 rounded-2xl border border-white/10 bg-white/6 px-4 py-3.5 backdrop-blur transition-all duration-400 hover:-translate-y-0.5 hover:border-sky-300/35 hover:bg-white/10"
              >
                <span
                  aria-hidden
          ...
          <Link
            href="/integrations"
            className="btn-shine mt-8 inline-flex min-h-12 items-center gap-2 rounded-full bg-white px-6 py-3 font-numeric text-sm font-semibold text-[#0b1f4a] shadow-[0_14px_32px_-16px_rgba(255,255,255,0.35)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-sky-50"
          >
            {t("cta")}
