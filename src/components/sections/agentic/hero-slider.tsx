...
export function HeroSlider({ copy }: { copy: HeroSliderCopy }) {
  ...
  const go = (target: number) => {
    setIndex((target + slideCount) % slideCount);
  };
    ...
    <div className="grid items-center gap-12 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:gap-10 xl:gap-14">
      <div className="hero-stagger text-center lg:text-left">
        <p
          ...
          className="hero-slide mb-6 inline-flex items-center gap-2.5 rounded-full border border-sky-400/30 bg-sky-400/10 px-4 py-1.5 font-numeric text-[0.7rem] font-semibold tracking-[0.14em] text-sky-200 uppercase backdrop-blur"
        >
          <span
            aria-hidden
        ...
        <div key={`copy-${active.key}`} className="hero-slide">
        ...
        {/* Manual slide controls — arrows only, no autoplay, no tab rail. */}
        <div className="mt-8 flex items-center justify-center gap-2 lg:justify-start">
          <button
            type="button"
            aria-label={copy.previousLabel}
            onClick={() => go(index - 1)}
            className="grid size-10 cursor-pointer place-items-center rounded-full border border-white/20 bg-white/8 text-white transition-all hover:border-white/50 hover:bg-white/15 focus-visible:ring-3 focus-visible:ring-sky-400/70 focus-visible:outline-none"
          >
          ...
          <span
            aria-live="polite"
            className="ml-1 font-numeric text-[0.7rem] font-semibold tracking-[0.14em] text-sky-200/80 tabular-nums"
          >
        ...
        <div className="relative overflow-hidden rounded-[2rem] border border-white/12 bg-linear-to-b from-white/10 to-white/4 p-2 shadow-[0_50px_90px_-40px_rgba(0,0,0,0.85),0_0_0_1px_rgba(255,255,255,0.04)_inset] backdrop-blur-xl sm:p-2.5">
          ...
          {active.visual ? (
            <div
              ...
              className="hero-slide relative h-[min(680px,78svh)] w-full overflow-hidden rounded-[1.55rem] bg-linear-to-b from-white/8 to-white/2"
            >
              {/* Room for the stage badge so UI chrome stays readable. */}
              <Image
                src={active.visual}
                alt=""
                fill
                sizes="(max-width: 640px) 100vw, (max-width: 1024px) 90vw, 52rem"
                className="object-contain object-center px-2 pt-10 pb-3 sm:px-4 sm:pt-12 sm:pb-4"
                quality={75}
              />
            </div>
          ) : (
          ...
          <div className="absolute top-5 left-5 z-30 flex max-w-[calc(100%-2.5rem)] items-center gap-2 rounded-full border border-white/20 bg-ink-panel/90 py-1.5 pr-3.5 pl-2.5 shadow-[0_12px_28px_-16px_rgba(0,0,0,0.9)] backdrop-blur-md">
            {active.icon ? (
            ...
            <span
              ...
              className="hero-slide truncate font-numeric text-[0.68rem] font-semibold tracking-[0.14em] text-sky-100 uppercase"
            >
              {active.stageLabel}
            </span>
          </div>
