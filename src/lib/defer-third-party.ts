...
function run(){
  ...
  for(var i=0;i<events.length;i++)w.removeEventListener(events[i],run);
  start();
}
for(var i=0;i<events.length;i++)w.addEventListener(events[i],run,{passive:true,capture:true});
})(window,document);`;
}
